...
addon = xbmcaddon.Addon('plugin.video.RETROCAVERNA-MEDIA')
...
SOURCES = [{"url":"https://dl.dropbox.com/s/uo5c7lcrnzcmgng/PRINCIPALRETROADDONS.xml.xml?", "fanart": "https://dl.dropbox.com/s/v9p34nw3rtez5a0/RETROCAVERNA%20LOGO.jpg?", "genre": "Tv Live", "date": "25.11.2015", "credits": "ruben pirez cristian veron", "thumbnail": "https://dl.dropbox.com/s/ncqyd7jxj48u909/LOGO%20RUFO.jpg?"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.RETROCAVERNA-MEDIA-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(RETROCAVERNA-MEDIA,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(RETROCAVERNA-MEDIA,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR yellow][B]- FAVORITES CHANNEL RETROCAVERNA-MEDIA[/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
